...
public class exitmotorcode {
    public void exitMotors() throws IOException{
        ...
        for(int i = 0; i <= MCFileNum; i++){
            ...
            String type = motorProp.getProperty("motorType");    
            if (type == "talonSRX"){
                talonMotor = loadTalon(motorFiles.toString());
                talonMotor.
            }
            if (type == "victorSPX"){
                victorMotor = loadVictor(motorFiles.toString());
                victorMotor.();
            }
        }
    }
